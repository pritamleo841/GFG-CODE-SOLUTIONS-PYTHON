    ...
    def Ancestors(self, root, target):
        ...
        def preorder(node):
            ...
            if preorder(node.left) or preorder(node.right):
                path.append(node.data)  # Add ancestor when unwinding recursion
                return True
            return False  # Return False if target is not found in this path
            
        preorder(root)
        return path  # List of ancestors in root-to-leaf order
