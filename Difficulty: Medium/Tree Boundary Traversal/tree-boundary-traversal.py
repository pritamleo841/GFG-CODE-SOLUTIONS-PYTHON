...
class Solution:
    def boundaryTraversal(self, root):
        ...
        # Collects left boundary nodes (top-down)
        ...
        # Collects leaf nodes (in left-to-right order)
        def leafBoundary(node, res):
            if node is None:
                return
            if isLeaf(node):
                res.append(node.data)
        ...
        # Collects right boundary nodes (bottom-up)
        def rightBoundary(node, res):
            ...
            res.append(node.data)  # Append after recursion for bottom-up order
